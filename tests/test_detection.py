...
import unittest
import numpy as np
from detection.utils import generate_window_sizes
...
class PipelineTests(unittest.TestCase):
    ...
    def test_scan_thresholds_and_alignment(self):
        config = json.loads((Path(__file__).resolve().parents[1] / 'configs/example.json').read_text())['cpd']
        config['scan']['n_boot'] = 19
        rng = np.random.default_rng(7)
        values = np.r_[rng.normal(0, .1, 150), rng.normal(4, .1, 150)]
        report = detect_statistics({'shifted': values, 'constant': np.ones(300)},
                                   np.arange(300) * 2 + 128, config)
        for metric in report.values():
            ...
            for threshold in ['0.25', '0.4', '0.5', '0.6']:
                ...
                previous = cps
        self.assertEqual(report['constant']['detections']['0.25']['source_indices'], [])
